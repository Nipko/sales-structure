...
- **Mensajes entrantes = cola**: todo webhook **encola y recién entonces confirma** (`InboundQueueService`, cola `inbound-messages`). Nunca llamar `processIncomingMessage` directo desde un controller: un reinicio mataría el turno sin nada que reintentar. El turno es idempotente vía índice único `messages.external_id` + marcador `turn:done:*` + `dedupeId` del saliente. **`InboundQueueService.enqueue` valida y descarta** lo estructuralmente inválido (`tenantId`/`contactId`/`channelType`/`channelAccountId` vacíos) — es la frontera común a los 7 productores, así que la validación va ahí y no en el webhook de turno. Descarta con `return`, nunca `throw`: lanzar propaga un 5xx y el proveedor redelivera el mismo cuerpo roto en bucle. **Nunca coercionar `contactId` a `''`**: el índice único `(channel_type, external_id)` fusionaría a todos los remitentes desconocidos en UN contacto, con la IA respondiendo con el historial de otra persona. Y esa validación **no puede exigir `direction === 'inbound'`**: la cola también transporta los salientes de solo-guardado de coexistencia (`waba_echo`, `historical`)
...
| **Onboarding: hoja de decisiones (16-sep-2026; §5 = estado de implementación del 17-sep: D1/D15 guardado inmediato + modo revisado opcional, D2 esenciales, D7 etapa `live`, D8 tarjeta de 4, D16 nace sin asignaciones, D5/D18 academias con receta, plomería del trabajo perdido; y la lista de olas pendientes)** — qué quedó, cambió o se descartó de las ideas del agente anterior (LEGO, sandbox, estación de reanudación, superpoderes), 10 ideas nuevas que reutilizan lo que existe (historial de coexistencia para prellenar, foto del menú/precios, nota de voz, ensayo con cliente simulado, enlace compartible, cambiar hablando, salud semanal por WhatsApp) y 26 decisiones en modo de opciones A/B/C con recomendación y consecuencia | `docs/onboarding-decisions-2026-09.md` |
| **⭐ DISEÑO DE LA EXPERIENCIA DE CONFIGURACIÓN GUIADA (16-sep-2026) — la experiencia objetivo, no una lista de arreglos**: la promesa ("en 10 min cualquiera ve a su agente responder por el canal que usa, con ejemplos ya escritos para su rubro, y cambia cualquier cosa tocándola"), la anatomía de cada paso (pregunta + una línea de por qué + ejemplo completo prellenado + Usar así/Cambiar + sugerencias en chips + victoria visible), la receta del negocio como motor de sugerencias (esquema por industria/subtipo, generada por el modelo para "Otro", regla de espacios en blanco en vez de datos inventados), el día 0 en 6 pasos con los 5 canales en pie de igualdad y página de prueba pública del chat web, el después como tarjetas en el mismo lenguaje, el panel de salud de 4 preguntas, la matriz multicanal, qué existe vs qué se construye, 6 decisiones nuevas (D9-D14) y la lista de jerga prohibida | `docs/onboarding-experience-design-2026-09.md` |
| **⭐ DIAGNÓSTICO DEL ONBOARDING tras el video real (16-sep-2026) — EMPEZAR ACÁ antes de tocar el onboarding, el editor del agente o el panel de salud**: por qué una dueña con soporte en la llamada terminó en 48 min sin WhatsApp, sin una respuesta y con 15 min de edición perdidos en silencio; 5 causas raíz con archivo:línea (nacimiento genérico en "Otro" + 5 canales sembrados; las capas de borradores/candidatos/misión del 6-14-sep encima del día 0; 5-9 superficies de guía por pantalla; el muro de Meta sin triage; la cadena regla vacía → `agent_invalid` → `fields` filtrado → toast verde → sin `beforeunload`); 26 refutaciones, análisis fotograma a fotograma, referentes verificados con URL; diseño objetivo con las tres cosas separadas (día 0 de 4 pantallas, editor como editor, panel de salud de 4 preguntas), Ola 0 de plomería y 8 decisiones del dueño. Evidencia cruda en `docs/onboarding-diagnosis-2026-09-workdir/` | `docs/onboarding-diagnosis-2026-09.md` |
| **⭐ Assist veraz, aviso crítico consecuente y onboarding "configurable por cualquiera" (sep 2026, IMPLEMENTADO). Parte I: una sola definición de "conectado" compartida por Calidad y Canales, `channel_coverage` para el canal asignado sin conectar, barra de contexto en la pantalla destino y Assist con la lista real de canales. Parte II: `onboardingStage` único, asistente de 3 pasos, tarjeta de puesta en marcha como única fuente de progreso, 15 recorridos guiados y "Mostrarme cómo" en la ayuda. Incluye las 3 revisiones adversariales y los 24 defectos que encontraron** | `docs/assist-quality-guided-tours-plan-2026-09.md` |
...
| **Pricing / rentabilidad (precios COP por país, márgenes)** — ⚠️ la línea "WhatsApp/Meta" de su COGS no existe como costo nuestro (Parallly es Tech Provider: el tenant le paga a Meta); ver el doc siguiente | `docs/plan-profitability-2026-07.md` |

...
| **⭐ Auditoría del sistema de pagos (31-ago-2026): crash del snapshot mensual (COMMERCIAL_PAYMENTS con relación inexistente — el spread esquiva a tsc; fix con `satisfies`), refund inline muerto por casts `::uuid`, alcance comercial `is_internal` unificado entre `/admin/financials` y `/admin/tenants`, y el veredicto del motor Wompi/fiscal/cupones/retiro-MP (sólido, con evidencia). Pendientes: mezcla de monedas COP/USD en revenue del snapshot + endurecimientos Stripe dormidos** | `docs/payments-audit-2026-08.md` |

...
- `internal/` — Service-to-service endpoint (POST /internal/inbound-message). Acepta entrantes **y** los salientes de solo-guardado de coexistencia (`metadata.source` = `waba_echo`|`historical`, que `processIncomingMessage` desvía a `storeOnlyMessage`): exigir `direction === 'inbound'` los rechazaba con 400 → 8 reintentos → failed, y dejó la coexistencia muda (los mensajes que el dueño manda desde su celular nunca llegaban al inbox)
...
**Billing & Finance**:
- `billing/` — Wompi (Colombia) vía motor de recurrencia propio; Stripe dormido para el riel internacional. **MercadoPago RETIRADO ago 2026** (adapter, config y scripts eliminados; el literal sobrevive solo para leer filas históricas). Subscription lifecycle (create/cancel/pause/resume/change). Webhook verification (HMAC-SHA256) + idempotency. Plan quotas enforcement. 5 billing email templates. Card tokenization for self-serve checkout. **Ciclo mensual/anual** (~15% desc anual): el anual congela su propio importe en `priceLocalOverrides[country].annual.amountCents` (Wompi no tiene catálogo remoto: el precio local ES el contrato). `billing-admin.controller.ts` (super_admin, billing-ops): reconciliación on-demand (`POST reconcile`, `POST tenants/:tenantId/reconcile`), refund inline (`POST payments/:paymentId/refund`), comp plans, auditoría de cambios de precio. `coupons.controller.ts` — **solo cupones `free_months`**: extienden el trial local en la MISMA transacción que el canje (`CouponsService.redeemForTenant`) y se rechazan si la suscripción tiene un mandato vivo del proveedor (`providerSubscriptionId`), y con el motor interno corren también `nextChargeAt` — extender solo la fecha local dejaba al scheduler cobrando igual → doble cobro. Los tipos `percent_off`/`amount_off` quedan en la tabla como historial pero no se emiten ni se canjean: nunca descontaron nada del cobro real. Canjeable desde el alta (`couponCode` en `POST /auth/complete-onboarding`) y desde Configuración → Facturación. `sms-checkout.*` NEUTRALIZADO (cobraba con MP; SMS está apagado)
- `billing/adapters/wompi.adapter.ts` — IPaymentProvider + IChargingProvider (fuentes de pago guardadas, cobro por transacción, firma de integridad y checksum de webhook)
- `billing/recurring/` — motor propio: scheduler, cobro, polling, dunning, prorrateo, fuentes de pago
- `billing/webhook.controller.ts` — POST /billing/webhook/:provider (allowlist: wompi, stripe; verify + dispatch)
...
- `billing/payment-provider.factory.ts` — Resuelve el adapter por nombre. Sin default silencioso: un proveedor sin adapter (mercadopago, retirado) falla fuerte en vez de cobrar con ids ajenos

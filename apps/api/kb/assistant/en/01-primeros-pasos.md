...
1. **Your agent** — Parallly prepares a foundation for your industry and shows it in four cards: **what you offer**, **where and when you serve**, **how customers buy or book**, and **frequently asked questions**. Suggested prices are identified as examples and answers with missing details are not presented as ready. You can select **Use this foundation**, adjust the name and greeting, and try suggested questions in the chat beside it. If an answer is wrong, **Correct this answer** saves it as a canonical FAQ the agent can use. After the first real reply, **Change template** returns. What you save is applied immediately.
...
A plan change does not silently turn the link into live customer service. In **Administration → Channels**, the row labelled **Nothing to connect** lets you explicitly choose whether the link remains a **test** or becomes **customer service**; the second option requires a plan that includes web chat. That choice controls quotas, human handoff, and metrics from the next message. The row also offers **Open** and **Copy link**.

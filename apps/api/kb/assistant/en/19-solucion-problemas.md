...
1. **Does the connection have an agent assigned?** Go to **AI Agent**. If you see a notice like "channels without an assigned agent", those connections are handled by your default agent, if it is active. If there is no active one, or two agents have the same connection assigned, nobody answers there: **Agent health** flags it as critical under **Every connected channel has an agent that replies**. Open the correct agent and, under **Connection assignment**, check the exact account it should handle. Remember: there's **one AI agent per connection**.
...
4. **Did the conversation stay with a person?** When the agent hands a conversation to your team, the AI pauses in that conversation while a person handles it; resolve it in the **Inbox** when you are done. Also check in **AI Agents** that the agent is switched on.
...
7. **Is it WhatsApp, with something stopping every reply from that number?** **Agent health** shows it in the **WhatsApp can deliver replies** check, with the reason: the number's **billing time zone** still needs confirming; Meta won't charge your WhatsApp account; the account has no payment method in Meta (from **1 October 2026** Meta charges your WhatsApp Business account for every delivered service message and, without a payment method, stops delivering); or, with **Spend protection** on, the currency Meta bills in is not known yet. **If you are the admin**, open **Channels → WhatsApp**: confirm the number's time zone there, add or fix the payment method in Meta's tools and, if the **WhatsApp charges (Meta)** card says **Sending paused**, press **Resume sending**. **If you are a supervisor or an agent**, that screen does not open for you: the symptom is recognisable from the **Inbox** — WhatsApp stops going out while Instagram, Messenger or Telegram keep answering — and a supervisor also sees it in **Agent health**. Your part is to tell the admin, because the time zone, the payment method and resuming are theirs alone. Incoming messages keep being received in the meantime.

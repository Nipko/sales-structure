...
Votre agent IA est le « vendeur virtuel » qui répond à vos clients sur WhatsApp, Instagram, Messenger, Telegram, Email et le chat de votre site web, 24 h/24. Vous apprendrez ici à le créer, lui donner une personnalité, définir ses horaires et l'assigner à vos connexions.
...
## Combien d'agents votre plan inclut

| Plan | Agents IA | Enregistrer vos propres modèles |
|------|:---:|:---:|
| Emprendedor | 1 | Non |
| Starter | 1 | Non |
| Pro | 3 | Oui |
...
Si vous atteignez la limite, vous verrez l'avis **Limite d'agents atteinte** avec l'option **Améliorer le plan**.
...
3. Choisissez un modèle. Vous verrez trois groupes :
   ...
   - **Mes modèles** — ceux que vous avez enregistrés vous-même (disponible à partir du plan Pro).
...
- **Modèle IA** — le niveau de sophistication du « cerveau » de l'agent. Les plans supérieurs débloquent des modèles plus puissants.
...
Le nombre de connexions du même type dont vous pouvez disposer dépend de votre plan :
...
> Le plan Emprendedor inclut uniquement le canal WhatsApp. Les autres canaux se débloquent à partir de Starter (Telegram à partir de Pro).

## Que signifie l'avis « canaux sans agent assigné »

Si dans **Agent IA** vous voyez un avis mis en évidence du type « 2 canal(aux) sans agent assigné », cela signifie que vous avez des connexions actives qu'aucun agent ne prend en charge de façon spécifique. En attendant, ces messages sont traités par votre **agent par défaut**, avec une configuration générique.
...
- **Enregistrer comme modèle** — transforme la configuration en modèle réutilisable (il apparaît dans **Mes modèles**). Disponible à partir du plan Pro.
...
Oui, à partir du plan Pro (3 agents). Créez-en un avec le modèle **Conseiller Commercial** et un autre avec **Agent de Support**, puis assignez chacun à la connexion correspondante — par exemple, deux numéros WhatsApp différents.
...
Non. Dans Parallly, le SMS n'est pas un canal de conversation : il sert uniquement aux notifications sortantes avec des crédits (1 crédit = 1 segment). Les canaux conversationnels sont WhatsApp, Instagram, Messenger, Telegram, Email et le chat web.
...
Ces deux limites dépendent de votre plan (voir les tableaux ci-dessus). Vous pouvez améliorer votre plan depuis Paramètres → Facturation, ou nous écrire à https://parallly-chat.cloud/support si vous avez besoin d'une limite spéciale.

...
describe('Structured knowledge snapshot integration', () => {
    it.each(['structuredKnowledgeInputs', 'faqs', 'policies'])('rejects a missing %s before the model', async field => {
        ...
        await expect(f.service.test('tenant', 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa', { message: 'consulta' }, { agentSnapshot: snapshot }))
    ...
    it('rejects changed facts even if their inner integrity hash is resealed', async () => {
        ...
        await expect(f.service.test('tenant', 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa', { message: 'consulta' }, { agentSnapshot: snapshot })).rejects.toThrow('knowledge_integrity_mismatch');
        ...
        await expect(f.service.test('tenant', 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa', { message: 'consulta' }, { agentSnapshot: snapshot })).rejects.toThrow('frozen_dependencies_integrity_mismatch');
    });
    it('binds the collection to the source schema and preserves errors at capture', async () => {
        ...
        await expect(f.service.test('tenant', 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa', { message: 'consulta' }, { agentSnapshot: snapshot })).rejects.toThrow('knowledge_schema_mismatch');
        f.revisions.captureStructuredKnowledge.mockRejectedValue(new Error('source_query_failed'));
        await expect(f.service.captureSnapshot('tenant', 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa')).rejects.toThrow('source_query_failed');
    });
    it.each(['search_faqs', 'get_policy'])('binds %s to the server snapshot rather than model arguments', async name => {
        ...
        await f.service.test('tenant', 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa', { message: 'consulta' }, { agentSnapshot: snapshot });

...
export class PromptAssemblerService {
    ...
    private buildContractLayer(): string {
        const rules = [
            ...
            // persona lleva nombre propio y el cliente no tiene forma de saber
            // qué es: nada en el contrato le decía al modelo que no lo negara.
            ...
            if (vc.domainContract) {
                ...
                if (dc.intents.length) {
                    ...
                    for (const intent of dc.intents) {
                        ...
                        if (intent.workflowClass) attrs.push(`class="${intent.workflowClass}"`);
                        if (intent.workflowId) attrs.push(`workflow="${this.attrEscape(intent.workflowId)}"`);
                        if (intent.workflowReadiness) attrs.push(`workflow_readiness="${intent.workflowReadiness}"`);
                        if (intent.workflowInitialState) attrs.push(`initial_state="${this.attrEscape(intent.workflowInitialState)}"`);
                        if (intent.workflowTerminalStates?.length) attrs.push(`terminal_states="${this.attrEscape(intent.workflowTerminalStates.join(','))}"`);
                        if (intent.requiredSlots?.length) attrs.push(`required_slots="${this.attrEscape(intent.requiredSlots.join(','))}"`);
                        if (intent.nextStateAuthority) attrs.push(`state_authority="${intent.nextStateAuthority}"`);
                        if (intent.defaultDeny !== undefined) attrs.push(`default_deny="${intent.defaultDeny ? 'true' : 'false'}"`);

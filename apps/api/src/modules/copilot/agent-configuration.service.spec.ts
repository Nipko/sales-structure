...
function harness() {
    let agent: any = { id: AGENT, version: 4, config_json: { persona: { name: 'Luna', greeting: 'Hola' }, tools: { restaurants: { enabled: true, token: 'PRIVATE' } } } };
    let ledger: any[] = [];
    ...
    const query = async (sql: string, params: any[]) => {
        ...
        if (sql.includes('WHERE id<>')) { otherVersion++; return [{ id: 'other' }]; }
        ...
        if (sql.startsWith('INSERT INTO agent_config_proposals')) {
            ...
            Object.assign(row, { agent_name: params[7] }); ledger.push(row); return [clone(row)];
        }
        ...
        if (sql.startsWith('UPDATE agent_config_proposals')) {
            ...
            Object.assign(row, { status: 'applied', applied_by: params[1], applied_version: params[2] }); return [clone(row)];
        }
    ...
    const prisma = { getTenantSchemaName: jest.fn().mockResolvedValue('tenant_scope'), ensureCanonicalTables: jest.fn(),
        ...
        transactionInTenantSchema: jest.fn(async (_schema, work) => {
            const before = { agent: clone(agent), ledger: clone(ledger), tenant: clone(tenant), otherVersion };
            try { return await work(query); } catch (error) { agent = before.agent; ledger = before.ledger; Object.assign(tenant, before.tenant); otherVersion = before.otherVersion; throw error; }
    ...
    return { service: new AgentConfigurationService(prisma as any, persona as any, assessment as any, events as any, capabilities as any, tenants as any), prisma, persona, assessment, events, sqlCalls, capabilities, tenants, otherVersion: () => otherVersion,
...
describe('reviewed agent configuration', () => {
    ...
    it('checks enabled tools on every assigned channel both when proposing and when applying', async () => {
        ...
        expect(h.agent().config_json.tools.faqs.enabled).toBe(true);
    });
    ...
    it('uses proposal and agent locks, exact digest, current version and an atomic audit record', async () => {
        ...
        expect(result).toMatchObject({ proposal: { status: 'applied', appliedVersion: 5 }, verification: 'verified' });
        expect(h.agent().config_json.persona.greeting).toBe(changes[0].value);
        expect(h.ledger()[0].applied_by).toBe(ACTOR.id);
        expect(h.sqlCalls.filter(call => call.sql.includes('FOR UPDATE'))).toHaveLength(3);
        expect(h.sqlCalls.find(call => call.sql.startsWith('UPDATE agent_personas'))).toMatchObject({ sql: expect.stringContaining('AND version=$4'), params: [AGENT, expect.any(String), 'Luna', 4] });
        expect(h.assessment.getAssessment).toHaveBeenCalledWith(TENANT, AGENT);
        expect(h.events.emit).toHaveBeenCalledWith('agent.version.updated', expect.objectContaining({ tenantId: TENANT, agentId: AGENT, proposalId: p.id }));
    });
    it('replays both request and application idempotently, including after application', async () => {
        ...
        expect(h.agent().version).toBe(5);
        expect(h.ledger()).toHaveLength(1);
        expect(h.sqlCalls.filter(call => call.sql.startsWith('UPDATE agent_personas'))).toHaveLength(1);
    ...
    it('reports a committed change separately from failed verification and repairs finalization on replay', async () => {
        const h = harness();
        const p = await h.service.propose(TENANT, AGENT, changes, ACTOR);
        ...
        await expect(h.service.apply(TENANT, p.id, p.digest, ACTOR)).resolves.toMatchObject({ proposal: { status: 'applied' }, assessment: null, verification: 'unavailable' });

...
export const EVALUATION_CONTEXT_READS = [
    ...
    { port: 'core.learning', boundary: 'sealed_release_plus_live_privacy', readers: 'Learning.getRuntimeExamples/readRelease/assertReleaseSourcesAvailable', sources: 'selected frozen release ID/hash but source learning_releases/sources/examples and tombstones re-read; revoked sources must remain a live guard' },
    { port: 'core.models', boundary: 'live_guarded_router', readers: 'sessionLlmRouter; IntentInterpreter; rewriteSearchQuery; normal response/tool loop/output guardrails; Knowledge reranker/embedding', sources: 'router registry/settings/env + live provider; read-only context skips affinity/stat persistence, external weights/fallback/circuit/clock are not frozen' },
    { port: 'canonical_commands', boundary: 'owned_namespace', readers: 'ToolExecutionControl; canonical appointments/education/gyms/repair/catalog ports', sources: 'namespace contacts/conversations/messages/ledger/terms/catalog/business rows; actor directory shadow; no external providers/events; source lease owner + metadata introspection remain live' },
    ...
    { port: 'simulation.runner', boundary: 'source_control_plane_plus_namespace', readers: 'Simulation.run/buildReplayScenarios/loadScenariosFromRun/buildSummary; synthetic generator/customer simulator/judge', sources: 'source conversations/messages for initial replay selection; source simulation_runs for previous results/scenarios/baseline; snapshot persisted; generated/customer text and judge live LLM; namespace agent operations' },
    { port: 'learning.evaluator', boundary: 'source_control_plane_plus_namespace', readers: 'LearningEvaluation.start/process/guard/replay/databaseEvidence/judge; Learning.createEvaluationSnapshot/evaluationRun/dependencyHash', sources: 'source learning releases, train/holdout sources/examples + erasure; frozen scenario+release selection; namespace DB/ledger outcome verification; live judge router' },
    ...
    { port: 'clock', boundary: 'live_unfrozen', readers: 'Date.now/new Date; SQL NOW/CURRENT_DATE; provider mirror freshness and holds', sources: 'OS/DB/provider clocks; only fixture date selection uses snapshot.capturedAt today' },
] as const;

...
const TEXT: Record<string, Record<OperationalNoticeKind, string>> = {
    es: {
        ...
        'appointment.operator_slack': 'Nueva cita registrada.',
        'analytics.threshold_alert': 'Una alerta analítica alcanzó su umbral.',
        'analytics.scheduled_report': 'Tu reporte analítico está listo.',
        ...
        'push.domain_event': 'Nueva actividad operativa.',
    },
    en: {
        ...
        'appointment.operator_slack': 'New appointment registered.',
        'analytics.threshold_alert': 'An analytics alert reached its threshold.',
        'analytics.scheduled_report': 'Your analytics report is ready.',
        ...
        'push.domain_event': 'New operational activity.',
    },
    pt: {
        ...
        'appointment.operator_slack': 'Novo agendamento registrado.',
        'analytics.threshold_alert': 'Um alerta analítico atingiu seu limite.',
        'analytics.scheduled_report': 'Seu relatório analítico está pronto.',
        ...
        'push.domain_event': 'Nova atividade operacional.',
    },
    fr: {
        ...
        'appointment.operator_slack': 'Nouveau rendez-vous enregistré.',
        'analytics.threshold_alert': 'Une alerte analytique a atteint son seuil.',
        'analytics.scheduled_report': 'Votre rapport analytique est prêt.',
        ...
        'push.domain_event': 'Nouvelle activité opérationnelle.',
    },
};
export function operationalNoticeText(kind: OperationalNoticeKind, lang: string | undefined, facts: {
    name?: string; when?: string; service?: string; customer?: string; phone?: string;
    address?: string; problem?: string;
}): string {
    const text = (TEXT[String(lang || 'es').slice(0, 2).toLowerCase()] || TEXT.es)[kind];
    if (kind !== 'home_service.emergency') return [text, facts.name, facts.when].filter(Boolean).join('\n');
    const labels = ({
        es: ['Servicio','Cliente','Teléfono','Dirección','Problema'],
        en: ['Service','Customer','Phone','Address','Issue'],
        pt: ['Serviço','Cliente','Telefone','Endereço','Problema'],
        fr: ['Service','Client','Téléphone','Adresse','Problème'],
    } as Record<string,string[]>)[String(lang || 'es').slice(0,2).toLowerCase()] || ['Servicio','Cliente','Teléfono','Dirección','Problema'];
    return [text,`${labels[0]}: ${facts.service || '—'}`,`${labels[1]}: ${facts.customer || '—'}`,
        `${labels[2]}: ${facts.phone || '—'}`,`${labels[3]}: ${facts.address || '—'}`,
        `${labels[4]}: ${facts.problem || '—'}`].join('\n');
}

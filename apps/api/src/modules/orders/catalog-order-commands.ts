...
export class CatalogOrderCommands {
     ...
     *
     * `pending` is deliberately absent: an agent-placed order is created
     * `pending` because the business has not accepted it yet, and the seeded
    ...
    async create(schema: string, data: CatalogCreateInput, options: CatalogCommandOptions): Promise<any> {
        ...
        const committed = await this.transaction(schema, async query => {
    ...
    async advance(schema: string,id: string,next: string,expectedVersion?: number): Promise<void> {
        ...
        await this.transaction(schema,async query=>{
            ...
            const updated=await query<any[]>('UPDATE orders SET status=$1,version=version+1,updated_at=NOW() WHERE id=$2::uuid RETURNING *',[next,id]);

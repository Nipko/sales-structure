...
import { EVAL_IDENTITY_READERS, EVAL_IDENTITY_STEP_UP_WRITERS, evalStepUpWriterIsConfined } from './eval-identity-fixture';
...
const OPERATIONS: Record<string, { writer: string; family: string; table: string;
    where: Record<string, unknown>; scope?: Record<string, unknown> }> = {
    book_stay: {
    ...
    rent_vehicle: {
        ...
        where: {
            rental_type: 'vehicle_rental', resource_id: ids.vehicle,
            start_date: { op: 'date_eq', value: fixture.date }, end_date: { op: 'date_eq', value: fixture.endDate },
            // `pending_review`, no `reserved`: es la única de las siete que no
            // toma el recurso, porque identidad, licencia, seguro y pago los
            // revisa una persona.
...
describe('service, lodging and food operation competence pack', () => {
                ...
                for (const language of EVAL_LANGUAGES) {
                    ...
                    expect(untouched.expectedActions).toEqual([
                        ...
                        { kind: 'db_effect', type: 'no_row', family: operation.family, table: operation.table,
                            ...(operation.scope ? { where: operation.scope } : {}) },
                    ]);
                }
            }
        }
        // 7 servicios a domicilio + 4 fotografía + 4 restaurantes + 2 turismo
        // + 2 alojamiento + 2 guardería + 1 alquiler de vehículo.
    ...
    it('asserts the columns the production command persists, not merely that some row appeared', () => {
        for (const [key, operation] of Object.entries(OPERATIONS)) {
            ...
            // Contado por el tipo de fila cuando la tabla es compartida: dos
            // operaciones escriben `resource_rentals`, y contar la tabla entera
            // hacía que una rompiera la afirmación de la otra.
    ...
    it('admits each writer to the namespace and keeps it out of a real tenant schema', () => {
        ...
        // El alquiler de vehículo entra, y sigue siendo el único con identidad
        // escalada. Lo que cambió no es el control: es que la constancia
        // sintética del arriendo cubre ahora a los writers que ese arriendo
        // confina por completo, y la confinación se comprueba —canónico y de
        // familia `canonicalOnly`— en vez de suponerse.
        expect(CANONICAL_EVAL_TOOLS.has('create_vehicle_rental')).toBe(true);
        expect(EVAL_IDENTITY_READERS.has('create_vehicle_rental')).toBe(false);
        expect(EVAL_IDENTITY_STEP_UP_WRITERS.has('create_vehicle_rental')).toBe(true);
        expect(evalStepUpWriterIsConfined('create_vehicle_rental')).toBe(true);
        ...
        // Y ninguna de las otras seis la necesita, que es por qué el alquiler
        // fue el último en entrar.
        for (const writer of Object.values(OPERATIONS)) {
            if (writer.writer === 'create_vehicle_rental') continue;
            expect(ASSURANCE_LEVEL_MATRIX[TOOL_POLICY_REGISTRY[writer.writer].assurance].requiresStepUpIdentity).toBe(false);
        }
        // `file_claim` no entra por la misma puerta: su familia existe para
        // demostrar la NEGACIÓN del step-up, así que no hay writer que cubrir.
        expect(EVAL_IDENTITY_STEP_UP_WRITERS.has('file_claim')).toBe(false);
        expect(evalStepUpWriterIsConfined('file_claim')).toBe(false);
    });

    it('clears the missing-positive gap for the seven operations without claiming certification', () => {
        ...
        expect(tasks).toBe(22);
        // Quedan cinco, y los cinco son `file_claim`. No es un hueco de
        // escenario ni de identidad sintética: en una evaluación ese comando
        // existe para demostrar que el step-up lo RECHAZA, así que nunca llega
        // a un writer y pedirle un caso positivo sería pedir que se compruebe
        // que no se escribió nada.
        expect(matrix.summary.tasksMissingPositiveCases).toBe(5);
        ...
        expect(pending.sort()).toEqual(['seguros/aseguradora/file_claim',

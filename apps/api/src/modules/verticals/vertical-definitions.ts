...
const FINANZAS = createGenericVertical('finanzas', {
    ...
    // tasa y estado de la solicitud— no estaban, así que el agente arrancaba
    // sin nada que responder sobre lo único que el cliente pregunta.
    //
    // Las respuestas NO prometen aprobación ni rendimiento: las reglas del
    // agente lo prohíben explícitamente y una FAQ sembrada que lo contradiga
    // pesa más que la regla, porque entra por RAG como si fuera un hecho.
    faqs: [
        { question: { es: '¿Qué requisitos piden?', en: 'What are the requirements?', pt: 'Quais são os requisitos?', fr: 'Quelles sont les conditions?' }, answer: { es: 'Los requisitos cambian según el producto y tu perfil. Cuéntame qué necesitas y un asesor te confirma exactamente qué se pide en tu caso.', en: 'Requirements vary by product and profile. Tell me what you need and an advisor will confirm exactly what applies to your case.', pt: 'Os requisitos variam conforme o produto e o perfil. Conte o que precisa e um consultor confirma o que se aplica ao seu caso.', fr: 'Les conditions varient selon le produit et le profil. Dites-moi ce dont vous avez besoin et un conseiller vous confirmera ce qui s\'applique.' }, category: 'requisitos' },
        { question: { es: '¿Cuánto tarda la aprobación?', en: 'How long does approval take?', pt: 'Quanto tempo leva a aprovação?', fr: 'Combien de temps prend l\'approbation?' }, answer: { es: 'Depende del producto y de qué tan completa esté la documentación. Un asesor te da el plazo estimado de tu solicitud; ninguna aprobación está garantizada de antemano.', en: 'It depends on the product and how complete your documents are. An advisor will give you the estimated timeline; no approval is guaranteed in advance.', pt: 'Depende do produto e de quão completa está a documentação. Um consultor informa o prazo estimado; nenhuma aprovação é garantida de antemão.', fr: 'Cela dépend du produit et de vos documents. Un conseiller vous donnera le délai estimé; aucune approbation n\'est garantie à l\'avance.' }, category: 'tiempos' },
        { question: { es: '¿Qué documentos tengo que enviar?', en: 'What documents do I need?', pt: 'Quais documentos preciso enviar?', fr: 'Quels documents dois-je fournir?' }, answer: { es: 'Te pasamos la lista exacta según el producto. Por seguridad, los documentos se entregan por el canal formal que te indique el asesor, nunca por chat abierto.', en: 'We will send the exact list for your product. For security, documents are submitted through the formal channel your advisor indicates, never over open chat.', pt: 'Enviamos a lista exata conforme o produto. Por segurança, os documentos vão pelo canal formal que o consultor indicar, nunca por chat aberto.', fr: 'Nous vous enverrons la liste exacte. Par sécurité, les documents passent par le canal formel indiqué par le conseiller, jamais par chat ouvert.' }, category: 'documentos' },
        { question: { es: '¿Cuál es la tasa?', en: 'What is the interest rate?', pt: 'Qual é a taxa?', fr: 'Quel est le taux?' }, answer: { es: 'La tasa se define según el producto, el plazo y la evaluación de tu perfil, así que no puedo darte una cifra cerrada por aquí. Te agendo con un asesor y te la confirma con todo por escrito.', en: 'The rate depends on the product, the term and your profile assessment, so I cannot quote a fixed figure here. I can book you with an advisor who will confirm it in writing.', pt: 'A taxa depende do produto, do prazo e da avaliação do perfil, então não posso dar um número fechado aqui. Agendo com um consultor que confirma por escrito.', fr: 'Le taux dépend du produit, de la durée et de l\'évaluation de votre profil. Je ne peux pas donner de chiffre ferme ici; un conseiller vous le confirmera par écrit.' }, category: 'tasas' },
        { question: { es: '¿Cómo va mi solicitud?', en: 'What is the status of my application?', pt: 'Como está minha solicitação?', fr: 'Où en est ma demande?' }, answer: { es: 'Con gusto la reviso. Pásame tu nombre y el documento con el que la radicaste y te digo en qué etapa está.', en: 'Happy to check. Send me your name and the ID you applied with and I will tell you which stage it is in.', pt: 'Com prazer verifico. Me passe seu nome e o documento com que abriu a solicitação e informo em que etapa está.', fr: 'Avec plaisir. Donnez-moi votre nom et le document utilisé lors du dépôt et je vous dirai où elle en est.' }, category: 'estado' },
...
const SERVICIOS_PROFESIONALES = createGenericVertical('servicios_profesionales', {
    ...
    //
    // Ninguna respuesta opina sobre el fondo del asunto: las reglas del agente
    // prohíben asesoramiento legal o fiscal directo, y el disparador de handoff
    // ya incluye "me demandaron", "audiencia" y "honorarios" — o sea que estas
    // FAQs cubren el tramo previo, no lo reemplazan.
    faqs: [
        { question: { es: '¿Cuánto cobran?', en: 'What are your fees?', pt: 'Quanto cobram?', fr: 'Quels sont vos honoraires?' }, answer: { es: 'Los honorarios dependen del tipo de asunto y su complejidad, así que se cotizan después de la primera reunión. Ahí te entregamos la propuesta por escrito, sin compromiso.', en: 'Fees depend on the type of matter and its complexity, so we quote after the first meeting. You get the proposal in writing, with no obligation.', pt: 'Os honorários dependem do tipo de assunto e da complexidade, por isso cotamos após a primeira reunião, com proposta por escrito e sem compromisso.', fr: 'Les honoraires dépendent du type de dossier et de sa complexité; nous les chiffrons après le premier rendez-vous, par une proposition écrite et sans engagement.' }, category: 'honorarios' },
        { question: { es: '¿La primera consulta es gratis?', en: 'Is the first consultation free?', pt: 'A primeira consulta é gratuita?', fr: 'La première consultation est-elle gratuite?' }, answer: { es: 'Te confirmo cómo funciona la primera reunión en nuestro caso y qué incluye. Si quieres, te la agendo ahora.', en: 'I can confirm how our first meeting works and what it covers. I can book it for you now if you like.', pt: 'Confirmo como funciona a primeira reunião no nosso caso e o que inclui. Se quiser, agendo agora.', fr: 'Je vous confirme le fonctionnement du premier rendez-vous et ce qu\'il comprend. Je peux le planifier tout de suite.' }, category: 'honorarios' },
        { question: { es: '¿Atienden este tipo de caso?', en: 'Do you handle this type of case?', pt: 'Vocês atendem esse tipo de caso?', fr: 'Traitez-vous ce type de dossier?' }, answer: { es: 'Cuéntame brevemente de qué se trata y te digo si es de las áreas que trabajamos. Si no lo es, te lo digo de una y no te hacemos perder tiempo.', en: 'Tell me briefly what it is about and I will tell you whether it falls within our practice areas. If it does not, I will say so right away.', pt: 'Conte brevemente do que se trata e digo se é uma das áreas que atendemos. Se não for, aviso na hora.', fr: 'Décrivez-moi brièvement la situation et je vous dirai si cela relève de nos domaines. Sinon, je vous le dirai immédiatement.' }, category: 'alcance' },
        { question: { es: '¿Cuánto tarda?', en: 'How long does it take?', pt: 'Quanto tempo leva?', fr: 'Combien de temps cela prend-il?' }, answer: { es: 'Los tiempos dependen del trámite y de terceros que no controlamos (juzgados, entidades). En la primera reunión te damos un estimado realista de tu caso.', en: 'Timelines depend on the procedure and on third parties we do not control (courts, agencies). At the first meeting we give you a realistic estimate for your case.', pt: 'Os prazos dependem do trâmite e de terceiros que não controlamos (tribunais, órgãos). Na primeira reunião damos uma estimativa realista.', fr: 'Les délais dépendent de la procédure et de tiers hors de notre contrôle (tribunaux, administrations). Nous donnons une estimation réaliste au premier rendez-vous.' }, category: 'tiempos' },
        { question: { es: '¿Cómo va mi caso?', en: 'How is my case going?', pt: 'Como está meu caso?', fr: 'Où en est mon dossier?' }, answer: { es: 'Puedo avisarle al profesional a cargo para que te actualice. Pásame tu nombre y de qué asunto se trata y lo escalo ahora mismo.', en: 'I can notify the professional handling it so they update you. Give me your name and which matter it is and I will escalate right away.', pt: 'Posso avisar o profissional responsável para que atualize você. Me passe seu nome e o assunto e escalo agora.', fr: 'Je peux prévenir le professionnel en charge pour qu\'il vous informe. Donnez-moi votre nom et le dossier concerné et je transmets immédiatement.' }, category: 'estado' },
    ],
    services: [
...
const TECHNOLOGY = createGenericVertical('technology', {
    ...
    agent: { name: { es: 'Ana', en: 'Ana', pt: 'Ana', fr: 'Anne' }, role: { es: 'Asesora tecnológica', en: 'Technology advisor', pt: 'Consultora tecnológica', fr: 'Conseillère technologique' }, tone: 'professional', formality: 'semi-formal', greeting: { es: 'Hola, soy Ana, asesora tecnológica. ¿En qué solución puedo ayudarte?', en: 'Hello, I\'m Ana, your tech advisor. What solution can I help with?', pt: 'Olá, sou Ana, consultora tecnológica. Como posso ajudar?', fr: 'Bonjour, je suis Anne, conseillère technologique. Comment puis-je vous aider?' }, rules: { es: 'Califica el nivel técnico del cliente. Ofrece demos. Para proyectos enterprise, escala.', en: 'Qualify client\'s technical level. Offer demos. For enterprise, escalate.', pt: 'Qualifique o nível técnico do cliente. Ofereça demos.', fr: 'Qualifiez le niveau technique. Proposez des démos.' }, forbiddenTopics: { es: 'Acceso a sistemas|Credenciales|Garantizar SLA sin autorización', en: 'System access|Credentials|Guarantee SLA without authorization', pt: 'Acesso a sistemas|Credenciais|Garantir SLA', fr: 'Accès systèmes|Identifiants|Garantir SLA' }, // Se evaluan por SUBSTRING sobre el texto del cliente, asi que
        // 'presupuesto >$50M' era literalmente inescribible: nadie tipea una
        // comparacion numerica. De los 8 originales, 6 eran frases de
        // categoria interna ("proyecto enterprise", "integracion compleja")
        // que ningun prospecto usa para describirse. Se reemplazan por lo que
        // SI se escribe en B2B — y "demo" y "hablar con ventas" son ademas las
        // dos que mas plata dejan sobre la mesa si las contesta un bot.
        handoffTriggers: { es: 'demo|demostracion|hablar con ventas|propuesta comercial|licitacion|incidente de seguridad|se cayo|no funciona el servicio|contrato marco', en: 'demo|talk to sales|speak with sales|commercial proposal|rfp|security incident|service is down|outage|master agreement', pt: 'demo|demonstracao|falar com vendas|proposta comercial|licitacao|incidente de seguranca|fora do ar', fr: 'demo|demonstration|parler aux ventes|proposition commerciale|appel d\'offres|incident de securite|hors service' } },
    pipeline: { stages: [
        { name: { es: 'Lead', en: 'Lead', pt: 'Lead', fr: 'Lead' }, slug: 'lead', color: '#2c3e50', probability: 5, isTerminal: false },
        { name: { es: 'Discovery', en: 'Discovery', pt: 'Discovery', fr: 'Découverte' }, slug: 'discovery', color: '#3498db', probability: 15, isTerminal: false, transitionRules: [{ type: 'name_required' }, { type: 'email_required' }] },
    ...
    // soporte y dónde quedan los datos— no estaba sembrado.
    //
    // "demo" y "hablar con ventas" ya son disparadores de handoff, así que la
    // FAQ de demo no intenta resolverla: la encamina hacia la persona, que es
    // donde está la plata de este rubro.
    faqs: [
 ...
 */
type VerticalRecipeOverlay = Partial<Pick<VerticalDefinition, 'faqs' | 'services' | 'terminology'>> & { agent?: Partial<VerticalDefinition['agent']> };

const ACADEMY_SERVICES: VerticalDefinition['services'] = [
    { name: { es: 'Clase de prueba', en: 'Trial class', pt: 'Aula experimental', fr: 'Cours d\'essai' }, description: { es: 'Primera clase para conocer la academia', en: 'First class to get to know the academy', pt: 'Primeira aula para conhecer a academia', fr: 'Premier cours pour découvrir l\'école' }, durationMinutes: 60, price: 0, currency: 'COP', category: 'clases' },
    { name: { es: 'Mensualidad de clases grupales', en: 'Monthly group classes', pt: 'Mensalidade de aulas em grupo', fr: 'Abonnement mensuel cours collectifs' }, description: { es: 'Clases grupales durante un mes', en: 'Group classes for one month', pt: 'Aulas em grupo durante um mês', fr: 'Cours collectifs pendant un mois' }, durationMinutes: 60, price: 0, currency: 'COP', category: 'clases' },
    { name: { es: 'Clase personalizada', en: 'Private class', pt: 'Aula particular', fr: 'Cours particulier' }, description: { es: 'Clase individual con un profesor', en: 'One-to-one class with a teacher', pt: 'Aula individual com um professor', fr: 'Cours individuel avec un professeur' }, durationMinutes: 60, price: 0, currency: 'COP', category: 'clases' },
];

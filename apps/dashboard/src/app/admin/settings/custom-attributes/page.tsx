...
const ENTITY_TYPE_VALUES = ["contact", "lead", "company", "conversation"] as const;
const DATA_TYPE_VALUES = ["text", "number", "date", "boolean", "list", "url"] as const;
...
export default function CustomAttributesPage() {
    const t = useTranslations("settings.customAttributesPage");
    const tc = useTranslations("common");
    const tHelp = useTranslations("help");
    const ENTITY_TYPES = ENTITY_TYPE_VALUES.map(value => ({ value, label: t(`entityTypes.${value}`) }));
    const DATA_TYPES = DATA_TYPE_VALUES.map(value => ({ value, label: t(`dataTypes.${value}`) }));
    ...
    async function handleSave() {
        ...
        try {
            ...
            if (res.success) { showToast(editingId ? t("toastUpdated") : t("toastCreated")); setModalOpen(false); loadAttributes(); } else { showToast(res.error || tc("errorSaving")); }
    ...
    return (
        <div className="p-8 max-w-[1100px] mx-auto">
            ...
            <div className="flex justify-between items-center mb-7">
                ...
                <button onClick={openCreate} disabled={!canCreate("customAttributes", attributes.length)} className="flex items-center gap-2 px-5 py-2.5 rounded-[10px] bg-primary text-white border-none cursor-pointer text-sm font-semibold disabled:opacity-50 disabled:cursor-not-allowed"><Plus size={16} /> {t("newAttribute")}</button>
            </div>
            ...
            <UpgradeBanner current={attributes.length} limit={getLimit("customAttributes")} resourceLabel={t("resourceLabel")} />

            <div className="bg-card rounded-[14px] border border-border overflow-hidden">
                {loading ? <div className="p-10 text-center text-muted-foreground">{tc("loading")}</div>
                    : attributes.length === 0 ? <div className="p-10 text-center text-muted-foreground"><Database size={32} className="mb-2 opacity-40" /><p>{t("emptyState")}</p></div>
                        : (
                            <table className="w-full border-collapse">
                                <thead><tr className="border-b border-border">{[t("table.key"), t("table.label"), t("table.type"), t("table.required"), t("table.options"), t("table.actions")].map(h => <th key={h} className="px-4 py-3 text-left text-[11px] font-semibold text-muted-foreground uppercase tracking-wide">{h}</th>)}</tr></thead>
                                <tbody>
                                    {attributes.map(attr => (
                                        <tr key={attr.id} className="border-b border-border">
                                            ...
                                            <td className="px-4 py-3">{attr.is_required ? <span className="px-2.5 py-0.5 rounded-md bg-[var(--success)]/10 text-[var(--success)] text-xs font-semibold">{tc("yes")}</span> : <span className="text-xs text-muted-foreground">{tc("no")}</span>}</td>
                ...
                <div className="fixed inset-0 z-[1000] bg-black/60 flex items-center justify-center" onClick={() => setModalOpen(false)}>
                    <div className="bg-secondary rounded-xl border border-border p-7 w-[480px] max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="flex flex-col gap-4">
                            <div><label className={labelCls}>{t("form.entityType")}</label><select value={form.entity_type} onChange={e => setForm(prev => ({ ...prev, entity_type: e.target.value }))} className={inputCls}>{ENTITY_TYPES.map(et => <option key={et.value} value={et.value}>{et.label}</option>)}</select></div>
                            <div><label className={labelCls}>{t("form.label")}</label><input value={form.label} onChange={e => handleLabelChange(e.target.value)} placeholder={t("form.labelPlaceholder")} className={inputCls} /></div>
                            <div><label className={labelCls}>{t("form.key")}</label><input value={form.attribute_key} onChange={e => setForm(prev => ({ ...prev, attribute_key: e.target.value }))} placeholder="fecha_de_cumpleanos" className={cn(inputCls, "font-mono text-muted-foreground")} /></div>
                            <div><label className={labelCls}>{t("form.dataType")}</label><select value={form.data_type} onChange={e => setForm(prev => ({ ...prev, data_type: e.target.value }))} className={inputCls}>{DATA_TYPES.map(dt => <option key={dt.value} value={dt.value}>{dt.label}</option>)}</select></div>
                            {form.data_type === "list" && <div><label className={labelCls}>{t("form.options")}</label><textarea value={form.options} onChange={e => setForm(prev => ({ ...prev, options: e.target.value }))} placeholder={t("form.optionsPlaceholder")} rows={3} className={cn(inputCls, "resize-y")} /></div>}
                            <div className="flex items-center gap-3">
                                ...
                                <span className="text-[13px] text-foreground">{t("form.requiredField")}</span>
                            </div>
                        </div>

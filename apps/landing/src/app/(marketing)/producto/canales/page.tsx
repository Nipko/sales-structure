...
const FEATURES = [
  ...
  { key: "channelsFeature2", icon: () => Icon.fingerprint("w-6 h-6") },
  { key: "channelsFeature3", icon: () => Icon.telegram("w-6 h-6") },
  { key: "channelsFeature4", icon: () => Icon.mail("w-6 h-6") },
  { key: "channelsFeature5", icon: () => Icon.inbox("w-6 h-6") },
] as const;

const USE_CASES = [
  { key: "channelsUseCase1", emoji: "\u{1F485}" },
  { key: "channelsUseCase2", emoji: "\u{1F4CB}" },
  { key: "channelsUseCase3", emoji: "\u{1F393}" },
] as const;
    ...
    <>
      <JsonLd
        data={breadcrumbJsonLd([
          ...
          { name: "Canales", url: "/producto/canales" },
        ])}
      />

      {/* Hero */}
      <section className="pt-12 pb-20 px-6 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-[600px] h-[600px] rounded-full blur-3xl pointer-events-none bg-[#25D366]/5" />
        <div className="mx-auto max-w-6xl relative">
          ...
          >
            ...
            <span className="flex items-center gap-2 px-4 py-1.5 rounded-full text-xs font-semibold bg-[#25D366]/10 text-[#25D366] border border-[#25D366]/20 w-fit mb-5">
              {Icon.inbox("w-4 h-4")}
              {t("channelsBadge")}
            </span>

            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold tracking-tight mb-5">
              {t("channelsHeroPrefix")}{" "}
              <span className="text-[#25D366]">{t("channelsHeroHighlight")}</span>
            </h1>
      ...
      <Section>
        <div className="text-center mb-12">
          <h2 className="text-3xl sm:text-4xl font-bold tracking-tight mb-4">
            {t("channelsFeaturesTitle")}
          </h2>
          <p className="text-text-secondary max-w-xl mx-auto">
            {t("channelsFeaturesSubtitle")}
          </p>
        </div>

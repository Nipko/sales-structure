...
const FEATURES = [
  ...
  { key: "bookingFeature2", icon: () => Icon.calendar("w-6 h-6") },
  { key: "bookingFeature3", icon: () => Icon.shield("w-6 h-6") },
  { key: "bookingFeature4", icon: () => Icon.mail("w-6 h-6") },
  { key: "bookingFeature5", icon: () => Icon.layers("w-6 h-6") },
] as const;

const USE_CASES = [
  { key: "bookingUseCase1", emoji: "\u{1FA7A}" },
  { key: "bookingUseCase2", emoji: "\u{1F3CB}\u{FE0F}" },
  { key: "bookingUseCase3", emoji: "\u{1F37D}\u{FE0F}" },
] as const;
    ...
    <>
      <JsonLd
        data={breadcrumbJsonLd([
          ...
          { name: "Reservas", url: "/producto/reservas" },
        ])}
      />

      {/* Hero */}
      <section className="pt-12 pb-20 px-6 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-[600px] h-[600px] rounded-full blur-3xl pointer-events-none bg-accent/5" />
        <div className="mx-auto max-w-6xl relative">
          ...
          >
            ...
            <span className="flex items-center gap-2 px-4 py-1.5 rounded-full text-xs font-semibold bg-accent/10 text-accent border border-accent/20 w-fit mb-5">
            ...
            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold tracking-tight mb-5">
              {t("bookingHeroPrefix")}{" "}
              <span className="text-accent">{t("bookingHeroHighlight")}</span>
            </h1>
      ...
      <Section>
        <div className="text-center mb-12">
          <h2 className="text-3xl sm:text-4xl font-bold tracking-tight mb-4">
            {t("bookingFeaturesTitle")}
          </h2>
          <p className="text-text-secondary max-w-xl mx-auto">
            {t("bookingFeaturesSubtitle")}
          </p>
        </div>

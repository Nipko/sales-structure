...
E2 ya integra identidad, horarios, región, plantilla multilingüe y política de exposición como contexto capturado en el núcleo de evaluación. La integración RAG administrada quedó registrada en `5f036d10`. El manifiesto integral sigue vigente; lectores comerciales, proveedores y tráfico concurrente limitan la reproducibilidad completa. Evidencia y siguientes pasos en [Captura de contexto del núcleo](audits/2026-09-07/evaluation-core-context-capture.md).

Las colecciones completas de FAQs y políticas ya están integradas en los lectores del runtime, conservando búsqueda PostgreSQL e invalidación global. Aquella tanda corrigió ranking y desempate de FAQs; no incluía la integración RAG posterior ni la réplica comercial aún pendiente. Pruebas y límites en [Captura de conocimiento estructurado](audits/2026-09-07/evaluation-structured-knowledge-capture.md).
...
La réplica relacional de RAG y su lector privado ya tienen evidencia con PostgreSQL/pgvector. El gestor de ciclo de vida, espacios reutilizables y referencias con propiedad/vencimiento quedó registrado en **`aed7f994`, con 55 pruebas**. **`5f036d10` integra su uso automático con snapshots, Agent Test/Eval/Simulation/Learning, candidatas, jueces y retención. Validación del índice exacto: TypeScript API pasa; 42 suites / 610 casos pasan, cero omitidos, incluidas 17 suites PostgreSQL / 273 casos.** Las baterías se solapan y no deben sumarse. Autorun conserva fallos recuperables y presupuesto diferido; invalida fuentes vencidas y protege la finalización ante ACK incierto. La liberación corresponde a la referencia exacta y ocurre fuera del fence de fuente; limpieza diferida o fallida, solicitudes sustituidas y persistencias inciertas pueden conservarla hasta su TTL fijo. El presupuesto agregado y rendimiento bajo carga continúan pendientes. Alcance en [Réplica de conocimiento](audits/2026-09-07/evaluation-knowledge-replica.md) y [Conocimiento administrado en evaluaciones](audits/2026-09-07/evaluation-managed-knowledge-runtime.md).

**`ea868d15` incorpora pagos a la autoridad operativa: 18 suites / 183 pruebas, incluidas 20 PostgreSQL/Prisma.** `create_payment_link` liga tenant/agente/versión/hash al intent, procesamiento, reserva y admisión. El COMMIT de admisión permite una sola tentativa, sin locks durante el POST; la retirada previa impide enviarla, mientras los recibos y settlements posteriores siguen siendo conciliables. Un ACK incierto no permite recrear el enlace. Hay **16 herramientas con esta guarda**, pero siguen siendo **15 writers auditados en sandbox**: pagos no se habilita en evaluación.
...
`e8cea802` incorpora autoridad operativa en la entrega aprobada de imágenes y enlaces de pago de Web Chat, con persistencia atómica y recuperación sin duplicar recibos confirmados. TypeScript API y **10 suites / 90 pruebas**, incluidas 65 PostgreSQL, pasan sobre el índice exacto. No cubre respuestas normales, despacho externo, handoff ni Flow. Evidencia y límites en [Entregas aprobadas de Web Chat](audits/2026-09-07/approved-webchat-agent-authority.md).

`92f9d2fa` extiende esa entrega con routing vigente por conexión y serialización de cambios de prioridad, sin usar la primera atribución de la conversación como origen de la propuesta. API TypeScript y **10 suites / 94 pruebas**, incluidas 69 PostgreSQL, pasan sobre el índice exacto. El [plan de salida normal](audits/2026-09-07/normal-agent-dispatch-authority-plan.md) concreta el trabajo pendiente para preservar versiones y fuentes en respuestas, recuperación y envíos externos.

`9e5016f8` corrige también la reanudación: selección actual de la conexión, scope original y vínculo tenant/esquema verificado en el contexto y hasta el COMMIT de finalización. API TypeScript y **16 suites / 195 pruebas** completadas sobre el índice exacto, incluidas 91 PostgreSQL. [Evidencia y límites](audits/2026-09-07/approval-resume-routing.md). `3d9c04ea` registra siete pruebas del contrato de composición de fuentes obligatorias y solicitadas, con TypeScript API; una validación adicional no puede reemplazar el control de RAG de la sesión.

`54379f90` extrae la procedencia serializable del aprendizaje y sus comprobaciones a la misma conexión suministrada. El wrapper existente reutiliza la lectura; el modo de admisión conserva fuentes y versiones hasta COMMIT, todavía sin consumidor de mensajes. API TypeScript y **11 suites / 200 pruebas**, incluidas 86 PostgreSQL, pasan sobre el índice exacto sin omitidos. [Contrato y evidencia](audits/2026-09-07/runtime-learning-footprint.md). Esta infraestructura no acredita por sí sola una mejora semántica del agente.

La consolidación automotriz y los comandos de mascotas amplían las familias canónicas verificadas en PostgreSQL. Sus packs y lectores no acreditan todavía conversaciones completas con un modelo, identidad real por OTP ni entrega del proveedor. Permanecen en E3 la publicación HTTP, promoción/piloto/rollback integral y las demás fronteras de efectos y despacho diferido; en E2, lectores comerciales congelados, reloj coherente y evaluación bajo tráfico/carga. También siguen pendientes la mejora semántica del aprendizaje, UX/accesibilidad con usuarios nuevos, proveedores reales y comparación de desempeño. El diagnóstico que sigue corresponde a la base inicial indicada arriba, no a una nueva auditoría del estado posterior a esas correcciones.

...
Fecha: 2026-09-07. Alcance: AgentTest, Eval, Simulation y LearningEvaluation, su entrada común `ConversationsService.executeAgentTurn`, motores, capacidades, herramientas y lectores transitivos. Reconciliado contra `51886bb8`; no certifica perfiles ni cambia las condiciones de publicación.

Contexto del núcleo, FAQs/políticas, temporalidad de fixtures, autoridad de fuentes de aprendizaje y Replay ya están integrados. Vehículos y mascotas amplían a quince los writers del adaptador canónico; tres lectores privados/adicionales conservan sus guardas. El capturador relacional RAG y su puerto privado están comprobados, pero AgentTest/Eval/Simulation/Learning todavía no crean ni seleccionan automáticamente esa réplica. Gestión, slots, referencias de uso y presupuesto agregado de su ciclo de vida permanecen en implementación separada, sin validación integrada. El capturador piloto de servicios también sigue sin conectar y el manifiesto global se conserva completo.

Las pruebas de cada etapa están en [Contexto del núcleo](evaluation-core-context-capture.md), [Conocimiento estructurado](evaluation-structured-knowledge-capture.md), [Temporalidad](evaluation-temporal-context.md), [Réplica RAG](evaluation-knowledge-replica.md), [Retención de aprendizaje](learning-evaluation-retention.md), [Replay](simulation-replay-source-authority.md), [Vehículos](vehicle-appointment-consolidation.md) y [Mascotas](pet-command-lifecycle.md). El inventario TypeScript versión 6 describe las fronteras actuales; la evidencia original de la captura piloto se conserva abajo sin sumarla a las tandas posteriores.
...
| RAG automático/herramienta | Lector fuente por defecto; puerto privado opcional acepta réplica propia de documentos/embeddings/conflictos | Capturador y lectura canónica probados; lifecycle, selección automática, presupuesto, snapshot y retiro de réplica sin integrar; embedding/reranking conservan guardas vivas |
...
3. FAQ/policies ya capturan la colección elegible completa. Integrar el ciclo de vida y consumo automático del capturador RAG, con fuente/snapshot/lease, presupuesto y retiro; validar su gestión, slots y referencias de uso antes de habilitarlos. Búsqueda vacía y fuente nueva siguen siendo dependencias, no sólo el topK; conservar decisiones de conflictos y query/model/version.

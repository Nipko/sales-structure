...
**Estado del bloque 6 (Web Chat), 8 de septiembre:** resuelto e integrado. `64dc8d75` aporta el recibo canónico ligado al inbound y a la transición exactos; `d2ffdf41` integra el parche con la admisión autorizada por ese recibo en los siete puntos de escalada del turno; `02dc2e9e` añade su aceptación con PostgreSQL real. [Evidencia y límites](../2026-09-08/normal-widget-admission-and-handoff-receipt.md).

**Estado de los bloques 2 y 3, 8 de septiembre:** construidos y validados, sin productor. `d5d58216` aporta el outbox durable (una fila por efecto remoto, permiso de un intento, presupuesto que sobrevive al rollback); `4cf95267`, el transporte estricto con sus tres resultados y sin fallback de Flow; `c0ed2dc6`, la admisión con autoridad de conexión y fuentes en la misma conexión; `7ed373be`, el worker que entrega una fila con un solo intento; `3e7f78db`, la recuperación y el borrado. [Evidencia y límites](../2026-09-08/normal-dispatch-outbox.md).

**Sigue pendiente**: la adopción por `sendResponse` y los caches/replay de Conversations (bloque 3, segunda mitad), medios/enlaces/Flow como ítems propios con la división de Messenger (bloque 4), los canales distintos de WhatsApp, y la reconciliación operativa de los resultados inciertos. La secuencia y los criterios de las secciones siguientes se conservan tal cual.

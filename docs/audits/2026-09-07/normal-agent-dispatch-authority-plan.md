...
**Estado del bloque 6 (Web Chat), 8 de septiembre:** resuelto e integrado. `64dc8d75` aporta el recibo canónico ligado al inbound y a la transición exactos; `d2ffdf41` integra el parche con la admisión autorizada por ese recibo en los siete puntos de escalada del turno; `02dc2e9e` añade su aceptación con PostgreSQL real. [Evidencia y límites](../2026-09-08/normal-widget-admission-and-handoff-receipt.md). **Los bloques 1-5 y 7 de este plan siguen vigentes sin cambios**: el outbox de salida externa, el transporte estricto, los recibos por efecto remoto de medios/enlaces/Flow y la reconciliación no están implementados. La secuencia y los criterios de las secciones siguientes se conservan tal cual.

...
Revisión: `824b4c8d2ff313fe7865456e359a49ea98bb68dc`.

**El programa no está terminado.** 20 filas aceptadas, 24 bloqueadas por un
...
| M0 | **aceptada** | contador | — | Censo derivado del árbol: 25 call sites, 21 cobrables, 0 fuera de la autoridad económica. El inventario de efectos declara 63 productores y 43 con al menos una propiedad en `none`. La condición es que el artefacto versionado se regenere desde el mismo HEAD, no que alguien lo haya leído. | — |
...
| R0 | **aceptada** | contador | — | De 21 call sites cobrables, 0 usan un carril que no escribe fila antes del POST — Redis es el registro, o no hay registro. Un productor ahí no puede contestar "¿esto salió?" después de un reinicio. **Este número no es el criterio de R6**, que pregunta por la autorización aplicable y la mide con los productores fuera del gate económico. Lo que esta fila mide es la fila durable antes del POST; desde este HEAD todo productor cobrable la crea y ningún interruptor puede devolverlo al carril anterior. | — |
...
| Call sites de egress censados | 25 |
| De ellos, cobrables | 21 |
| Cobrables fuera del gate económico | 0 |
| Cobrables fuera del carril durable | 0 |
| Productores declarados en el inventario de efectos | 63 |

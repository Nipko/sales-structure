...
Revisión: `3909a3554c8c27cfbc619f6e598c82972c6f9636`.
...
| M0 | **aceptada** | contador | — | Censo derivado del árbol: 27 call sites, 23 cobrables, 0 fuera de la autoridad económica. El inventario de efectos declara 63 productores y 37 con al menos una propiedad en `none`. La condición es que el artefacto versionado se regenere desde el mismo HEAD, no que alguien lo haya leído. | — |
...
| R0 | **aceptada** | contador | — | De 23 call sites cobrables, 0 usan un carril que no escribe fila antes del POST — Redis es el registro, o no hay registro. Un productor ahí no puede contestar "¿esto salió?" después de un reinicio. **Este número no es el criterio de R6**, que pregunta por la autorización aplicable y la mide con los productores fuera del gate económico. Lo que esta fila mide es la fila durable antes del POST; desde este HEAD todo productor cobrable la crea y ningún interruptor puede devolverlo al carril anterior. | — |
...
| Call sites de egress censados | 27 |
| De ellos, cobrables | 23 |
...
| De ellos, con alguna propiedad en `none` | 37 |

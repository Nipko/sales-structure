...
const ROWS = [
    ...
    row('B1', { provenance: 'executed_evidence', artefact: 'docs/audits/2026-09-09/order-sensitivity-runs.md',
        open: channels.selfService - channels.implemented,
        openLabel: `${channels.selfService - channels.implemented} canales de autoservicio con capacidades locales pendientes`,
        gates: [1], evidence: `El turno completo corre de punta a punta sobre PostgreSQL, Valkey, BullMQ y Socket.IO reales, con crash en cada frontera, una erasure en vuelo y los primitivos a través de un PgBouncer real en modo transacción. La matriz registra ${channels.implemented} de ${channels.selfService} canales de autoservicio sin capacidad local pendiente; los límites que el proveedor no ofrece quedan fuera de alcance en vez de convertirse en estados inventados.` }),

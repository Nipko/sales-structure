...
> **Revisión del 18-sep-2026:** D1–D26 y la bitácora siguientes conservan su significado histórico. La [propuesta v2, sección 9](onboarding-experience-plan-v2-2026-09.md#9-enmiendas-propuestas-a-las-26-decisiones) documenta enmiendas recomendadas después de investigar productos, casos y usabilidad. No se marcan retroactivamente como aprobadas o construidas. Consultar la [auditoría técnica](onboarding-review-and-completion-plan-2026-09-18.md) para diferencias entre la bitácora y el código.
...
- **Ola 7 — una guía por pantalla, y cada pantalla dice la verdad (18-sep).** La grabación contó seis voces a la vez; esta ola deja una por pantalla mientras dure el día 0 y corrige lo que cada una prometía de más. El día 0 se pregunta siempre con `isOnboardingBeforeLive(stage, {firstReplyAt, createdAt})`, nunca con la etapa sola: el último botón del asistente escribe `completed`, que la etapa sola leía como "en vivo", y el panel se volvía a llenar al tocar "Ir al panel".
  - **Una guía por pantalla.** Asistente: sin franja de ayuda, sin "Cambiar plantilla" y sin "Mostrarme dónde" en el paso de conexión; vuelven tras la primera respuesta real. Inicio: mientras la tarjeta **Puesta en marcha** tenga algo que decir, es la pantalla (`homeCardOwnsScreen`): sin KPIs, franja de ayuda, Salud de agentes, actividad vacía, uso de IA ni agenda vacía; se retiraron el banner índigo "Retomar" y el ámbar de conectar, que decían lo mismo que la tarjeta con otro destino, y "Uso de modelos" dejó el "(Tier N)" que era solo la posición en la lista y la promesa "el router ahorra ~42%" que ningún número respaldaba. Lista de agentes: solo el aviso de configuración, en índigo (es algo por terminar, no algo roto: D3). Editor: sin franja de ayuda ni el resumen del pasaporte ("1 bloqueo crítico" encima de la caja de canales que decía lo mismo).
  - **Canales por receta y por plan.** El paso 2 pregunta "¿Por dónde te escriben tus clientes?" y ordena con `recipe.recommendedChannels`, con el motivo de la receta (un salón empieza por Instagram); sin receta, WhatsApp primero. El plan se lee antes de cualquier ventana: un canal que no incluye se muestra con candado e "Incluido desde {plan}", con el nombre tomado del catálogo en runtime (`GET /billing/plans`), nunca escrito en el código; si el recomendado está fuera del plan, el paso lo dice y lidera con uno incluido. "Listo" e Inicio nombran el mismo canal pendiente, con las mismas funciones (`orderWizardChannels` + `leadChannel`).
  - **El aviso de correo, antes de la ventana.** Instagram, Messenger y Telegram exigen correo confirmado en el API (`@RequiresVerifiedEmail`); el asistente lo dice arriba de esos canales, con "Reenviar código" y "Ya lo tengo", en vez de dejar recorrer la ventana de Meta para rechazar al final. WhatsApp no lo exige y el aviso lo aclara. Los rechazos se muestran como tarjetas con una acción, construidas desde el código y nunca con la frase de Meta o de Telegram: Telegram gana `invalid_bot_key` (antes "Token invalido" en texto, que `api.fetch` reducía a una tarjeta genérica) y el asistente lo conecta con `api.connectTelegram`, que conserva el código. Cada canal que no es WhatsApp tiene su propia confirmación ("Instagram conectado", cómo probarlo y el botón para abrirlo).
  - **Un "Listo" honesto.** Los ítems salen de `setup-status` (`hasKnowledge`, `hasTeam`): lo hecho se dice hecho, lo pendiente pendiente, y lo que no se pudo leer no se muestra ni como tarea ni como tilde. Nombra los canales conectados, y el pendiente es el que lideró el paso, no "Conectar WhatsApp" para todos.
  - **El enlace es un canal real en los planes con chat web.** `isTrialLink` decide por turno (gateway, configuración pública del widget y núcleo, el mismo predicado): sin chat web en el plan es una prueba (tope diario por página, cupo de por vida que paga la plataforma, sin humano); con chat web atiende con la cuota del plan, sin el tope diario, puede pasar al equipo y no se presenta como "prueba" ante los clientes. La tarjeta del enlace solo ofrece "Ponerlo en mi bio" y "Ponerlo en mi página web" cuando eso es cierto; en prueba dice para qué sirve y qué cambia con un plan con chat web. El mensaje de cupo agotado pasa de "cuando el negocio active su plan" (falso para quien ya paga un plan sin chat web) a "cuando el negocio active el chat web".
  - **Pantalla del cupo del enlace.** `/admin/demo-allowance` (solo super_admin, con su regla explícita en `dashboard-page-access.ts`: es un costo de la plataforma y no se alcanza impersonando): interruptor, respuestas de por vida por cuenta y tope por página y día, con techos (10.000 y 1.000) para que dos ceros de más no financien una página viral, validación que rechaza la edición entera nombrando cada campo, y auditoría con el operador real.
  - **La respuesta de WhatsApp vive en el servidor.** "¿Dónde vive hoy tu número?" se guarda en `tenants.settings.whatsappTriage` (`PUT /persona/:tenantId/whatsapp-triage`, clave reservada con su dueño) y viaja en `setup-status`; antes vivía en `localStorage` y "lo dejamos anotado" se rompía en otro celular. Inicio la usa: el paso del canal dice "Continuar donde quedaste" con el motivo y, para "otro proveedor" o "no lo tengo a mano", lleva a Canales → WhatsApp.
  - **La copia sigue al modo de guardado.** La línea de los interruptores del editor, el "Cómo funciona" de los módulos, la tarjeta de propuesta de Assist, la revisión de configuración y los recorridos y reglas que lee Assist decían "guarda el borrador y completa su publicación" a todos; ahora dicen lo que pasa en el modo del negocio (inmediato por defecto) y, si el modo no se pudo leer, una frase cierta en los dos.
  - **"Dime qué cambiar".** El editor tiene una sola forma de pedir un cambio en palabras: abre Assist con la petición, y la propuesta se aplica según el modo del negocio.
  - **El spec de jerga.** `i18n-parity.spec` recorre los namespaces del día 0 en los cuatro idiomas y rechaza la jerga del Apéndice A y el voseo. La API dejó de mandar voseo en los muros de plan de canales ("Actualiza tu plan…", con sus códigos de siempre).
  - **Assist (KB):** el artículo 01 en 4 idiomas describe el paso por receta y plan, el aviso de correo, "Continuar donde quedaste" y el enlace según el plan; `assistant-kb-contract.spec.ts` lee las etiquetas de los mensajes del panel y rechaza las frases viejas ("WhatsApp primero", "un aviso para retomar", "siempre disponible").

  **Lo que queda abierto de la Ola 7 — una decisión comercial:** la prueba sin tarjeta por defecto incluye solo WhatsApp, y la receta del vertical #1 (belleza) recomienda empezar por Instagram. Una dueña de salón en prueba lee "A tu negocio le conviene Instagram" con candado e "Incluido desde {plan}" y termina empezando por WhatsApp, el canal que su propia receta pone segundo. La pantalla ya lo dice con honestidad; lo que falta es decidir si la prueba incluye Instagram (con su costo y el correo confirmado que exige) o si la recomendación para quien está en prueba cambia.

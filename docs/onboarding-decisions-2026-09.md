...
1. **D10** columna `services.price_status` y que las tools no citen precios de ejemplo (hoy la receta de academia deja precio 0 en la clase de prueba y no inventa montos).
2. **D11 / D19** página pública `/w/{widgetId}` ("el enlace de {Nombre}") y su paso en el asistente.

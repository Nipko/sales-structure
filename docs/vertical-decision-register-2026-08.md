...
**Regla:** las recomendaciones fueron adoptadas explícitamente; su implementación y gates se detallan en [`vertical-decisions-implementation-2026-08.md`](./vertical-decisions-implementation-2026-08.md)
...
Cada recomendación quedó convertida en una política o contrato explícito. `Adoptada con contención` significa que la opción fue elegida, pero la ejecución peligrosa permanece deshabilitada hasta satisfacer su gate externo; no equivale a certificación. `Bloquea` conserva aquello que todavía no debe anunciarse ni activar.

## 2. Decisiones de alcance y posicionamiento

### DEC-01 — Orden real de certificación y conectores

- **Estado:** adoptada; anclas iniciales `restaurantes`, `turismo` y `servicios_hogar`, todas `implemented_not_certified`.
...
- **Estado:** adoptada; los tres permanecen presets horizontales honestos.
...
- **Estado:** adoptada con contención; fallback estable y builder v1 separado en preview, `applySupported:false`.
...
- **Estado:** adoptada con contención; preview actual preservado y contrato de sandbox efímero separado, sin provisión real todavía.
...
- **Estado:** adoptada parcial por capability; `web_widget` es formal para conversación automática y el handoff continúa bloqueado sin entrega humana.
...
- **Estado:** adoptada; registry exhaustivo por kind, con dominios sensibles y appointments regulados en modo tool-only A2.
...
- **Estado:** adoptada con contención; NBA exige lineage/freshness/eval y todo procesamiento multimodal permanece bloqueado hasta recibir consentimiento y retención server-authoritative.
...
- **Estado:** adoptada; matriz A0–A4 compartida y enforcement central, con A4 sujeto a ticket humano durable.
...
- **Estado:** adoptada; middleware central con ledger, confirmación ligada al turno/argumentos, assurance y workflow A4 durable de solicitud, decisión, reanudación única y recuperación fail-closed.

- **Hecho actual:** 92/92 tools estáticas están clasificadas y `getMissingToolControls()` reporta cero; tools MCP opacas se bloquean hasta registrar controles explícitos.
...
- **Estado:** adoptada con contención; contrato provider-neutral y ledger disponibles, ownership se liga a una referencia canónica y sin adapter explícito la operación deriva a handoff.
...
- **Estado:** adoptada como boundary canónico; moneda explícita/bloqueable y lineage FX existen, y cada writer debe migrar explícitamente al contrato.
...
- **Estado:** adoptada como contrato canónico; la migración de todos los writers e históricos sigue siendo un gate de datos.
...
- **Estado:** adoptada como modelo canónico; los bindings y recursos nuevos son explícitos, pero el booking legacy conserva `assigned_to` como UUID de usuario hasta completar su backfill.
...
- **Estado:** adoptada con contención; owner/provider/event/revision y outbox existen; al agotar retries queda una obligación explícita `reconciliation_required`, y reasignar cuentas permanece bloqueado hasta un workflow delete/upsert durable.

- **Baseline resuelto:** la cita ya persiste owner/provider/event/revision y los writers encolan en un outbox transaccional; el worker aplica lease, idempotencia y retry acotado. Un resultado ambiguo agotado bloquea revisiones posteriores y se muestra como reconciliación manual requerida. La reasignación entre cuentas sigue cerrada hasta implementar delete exacto en el owner anterior y upsert en el nuevo.
...
- **Estado:** adoptada con contención; preview/archive/outbox durables y apply fail-closed hasta registrar un adapter completo.
...
- **Estado:** adoptada incrementalmente; claims estructurados visibles usan registry positivo y el copy narrativo conserva denylist secundaria.
...
- **Estado:** adoptada parcialmente; existen 18 entradas versionadas con owner, fecha, expiración, tier/región/madurez y test interno, pero el snapshot/hash y la revisión humana de cada fuente siguen siendo gate.
...
- **Estado:** adoptada; workflow por tiers y artefactos JUnit/readiness, con gates externos de release todavía pendientes.
...
- **Estado:** adoptada; worker aislado con allowlist/deadline/memory/response cap y transporte Expo acotado.

- **Hecho:** Axios seguro y los transportes Expo/Web Push ya tienen DNS/host allowlist, deadline absoluto y límites de respuesta; falta validarlos contra infraestructura maliciosa controlada.
...
La implementación y los límites de cada adopción están consolidados en [`vertical-decisions-implementation-2026-08.md`](./vertical-decisions-implementation-2026-08.md). Los previews, bloqueos y fallos cerrados prevalecen hasta que la evidencia externa autorice una expansión.

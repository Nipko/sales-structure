...
### U71 — Gate 3: paquete autoral 1:1 y política de mercado

Se agregó `VerticalAuthoringPackageV1`, compuesto desde los registros ejecutables y no desde 76 copias de prompt. Cubre 76 configuraciones canónicas y cinco identidades legacy con procedencia por campo, las dimensiones obligatorias del plan desglosadas para auditoría, evals ES/EN/PT/FR, tools/Active Objects/SoR, menú/roles/móvil, templates y overlays de país. El dashboard de auditoría muestra la matriz completa y una matriz de 17 mercados con aliases y frases ambiguas que nunca consienten solas.

La medición derivada queda en 76/76 paquetes mecánicamente completos, 60 glosarios heredados, 70 perfiles sin reglas de persona explícitas por subtipo, 34 perfiles con gate regulatorio/dominio en el catálogo total, cero gaps de navegación y cero gaps de control de tools. Seis destinos de producto conservan `scope_without_committing_intent` y pasan a Fase 5; no se rellenaron con una tool vecina.
